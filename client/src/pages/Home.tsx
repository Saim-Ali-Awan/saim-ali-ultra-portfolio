  ...
  return (
    <main className="zajno-shell">
      ...
      <section id="work" className="zajno-section zajno-work">
        ...
        <div className="zajno-work__intro"><motion.h2 initial={{ y: 42, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true, amount: 0.35 }} transition={{ duration: 0.72, ease: [0.23, 1, 0.32, 1] }}>Work that<br /><em>moves.</em></motion.h2><p>From first principle to final polish, I build digital products that feel as good as they function.</p></div>
      ...
      <section id="playground" className="zajno-playground"><div className="zajno-section__head"><Label>Capabilities</Label><span>03</span></div><motion.h2 initial={{ y: 42, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.72, ease: [0.23, 1, 0.32, 1] }}>We dare to<br /><em>make it felt.</em></motion.h2><div className="zajno-services">{["Product design", "Web development", "Motion systems", "3D direction", "Creative technology"].map((service, index) => <div key={service}><small>0{index + 1}</small><span>{service}</span><ArrowUpRight size={18} /></div>)}</div><div className="zajno-tech-strip"><div className="zajno-marquee__track">{[...technologies, ...technologies].map((technology, index) => <span key={`${technology}-${index}`}>{technology}</span>)}</div></div></section>
